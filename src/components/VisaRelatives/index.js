...
class VisaRelatives extends Component {
  ...
  render() {
    ...
    const formItems = people.map((person, index) => (
      <Form.Item label={index === 0 ? label : ""} key={index} style={{ borderBottom: `1px dashed black` }} required>
        <Row gutter={16}>
          <Col xs={{ span: 24 }} sm={{ span: 24 }} md={{ span: 12 }}>
            <VisaSelectItem label={tr(resources.relatives.relatives.degree)} field={`${arrayField}[${index}].degree`} initialValue={initialValue[index] ? initialValue[index].degree : null} content={{ combines: constants.export_list(tr(constants.new_kdmid_relation_degree_list)) }} getFieldDecorator={getFieldDecorator} />
          </Col>
        </Row>
        <Row>
          <Col xs={{ span: 24 }} sm={{ span: 8 }} md={{ span: 8 }}>
            <VisaInput label={tr(resources.relatives.relatives.surname)} field={`${arrayField}[${index}].surname`} initialValue={initialValue[index] ? initialValue[index].surname : null} maxLength={30} getFieldDecorator={getFieldDecorator} />
          </Col>
          <Col xs={{ span: 24 }} sm={{ span: 16 }} md={{ span: 16 }}>
            <VisaInput label={tr(resources.relatives.relatives.firstname)} field={`${arrayField}[${index}].firstname`} initialValue={initialValue[index] ? initialValue[index].firstname : null} getFieldDecorator={getFieldDecorator} maxLength={60} required={false} />
          </Col>
        </Row>
        <Row>
        ...
        <Row>
          <Col xs={{ span: 18 }} md={{ span: 18 }} lg={{ span: 16 }}>
            <VisaInput label={tr(resources.relatives.relatives.address)} field={`${arrayField}[${index}].address`} initialValue={initialValue[index] ? initialValue[index].address : null} getFieldDecorator={getFieldDecorator} />
          </Col>
          {people.length > 1 ? <Icon className="dynamic-delete-button" type="minus-circle-o" style={{ fontSize: "24px", marginTop: "40px", marginLeft: "10px" }} onClick={() => this.remove(index, keysField, arrayField)} /> : null}
        </Row>
      </Form.Item>
    ));
