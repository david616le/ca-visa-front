...
class VisaInputArray extends Component {
  ...
  render() {
    ...
    const { label, getFieldDecorator, getFieldValue, setFieldsValue, initialValue, validators, keysField, arrayField, customRule, maxLength, required, lang, ...rest } = this.props
    ...
    const formItems = languages.map((lang, index) => (
      ...
      >
        ...
        })(<Input style={{ width: '50%', marginRight: 8 }} maxLength={maxLength}/>)}
        {languages.length > 1 ? (
          <Icon
    ...
    return (
      <>
        {formItems}
        <Form.Item>
          <Button type="dashed" onClick={() => this.add(keysField)} style={{ width: '50%' }}>
