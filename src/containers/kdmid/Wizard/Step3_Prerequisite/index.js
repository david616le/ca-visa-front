...
class MyForm extends Component {
  ...
  handleDates = (data) => {
    if(data.usaResidentCardExpiryDate)
      data.usaResidentCardExpiryDate = data.usaResidentCardExpiryDate.format('DD.MM.YYYY')
    return data
  }
  ...
  render() {
    ...
    const renderLawful = () => {
      ...
      if (isPermanentResidentOfUSAndHaveProofOfPermResidence !== '1' && !['32', '195'].includes(passportNotedNationality))
        bNext = true
      return (
        <>
          <VisaRadio
            label={tr(resources.step_prerequisite.isPermanentResidentOfUSAndHaveProofOfPermResidence)}
            initialValue={_.get(data, 'isPermanentResidentOfUSAndHaveProofOfPermResidence')}
            field="isPermanentResidentOfUSAndHaveProofOfPermResidence"
          ...
          {isPermanentResidentOfUSAndHaveProofOfPermResidence === '0' && (
            <>
              <Alert
                message="As a lawful permanent resident of the U.S."
                description={
                  <div>You must show proof of your United States lawful permanent resident card (USCIS) and travel with the passport you will use for this application to be able to board your flight to Canada.</div>}
                type="warning"
                showIcon
              />
              <Row>
                <Col sm={16}>
                  <VisaInput 
                    label={tr(resources.step_prerequisite.usaResidentCardNumber)}
                    field="usaResidentCardNumber"
                    initialValue={_.get(data, 'usaResidentCardNumber')}
                    getFieldDecorator={getFieldDecorator}
                    maxLength={9}
                    customRule={[{ validator: validators.validateUSAResidentCardNumber }]}
                  />
                </Col>
                <Col sm={16}>
                  <VisaInput 
              ...
              {passportNotedNationality === '32' && (
                <Alert
                  message="As a Canadian citizen, you cannot apply for an eTA"
                  description={
                    <div>By law, Canadian citizens have the right to enter Canada.  Find out how to apply for or renew a Canadian passport.</div>}
                  type="warning"
                  showIcon
                />
              )}
              {passportNotedNationality === '195' && (
                <Alert
                  message="As a U.S. citizen, you are exempt from the eTA requirement."
                  description={
                    <div>You do not need an eTA to travel to Canada. Make sure to travel with valid proof of U.S. citizenship such as a passport and other acceptable travel documents.</div>}
                  type="warning"
                  showIcon
                />
              )}
    ...
    switch(countryOfCitizenship) {
      ...
      case '92':
        // Israel
        if (getFieldValue('hasIsraeliNationalPassport') === '0' && getFieldValue('passportNotedNationality') === '83')
          bNext = true
        renderByCountryCode = (
          <>
            <VisaRadio
              label={tr(resources.step_prerequisite.hasIsraeliNationalPassport)}
              initialValue={_.get(data, 'hasIsraeliNationalPassport')}
            ...
            {getFieldValue('hasIsraeliNationalPassport') === '1' && (
              renderLawful()
            )}
          </>
        )
      ...
      case '158':
        // Romania
        if (getFieldValue('hasEPassport') === '0' && getFieldValue('passportNotedNationality') === '147')
          bNext = true
        renderByCountryCode = (
          <>
            <VisaRadio
              label={tr(resources.step_prerequisite.hasEPassport)}
              initialValue={_.get(data, 'hasEPassport')}
              field="hasEPassport"
              getFieldDecorator={getFieldDecorator}
            />
      ...
      case '193':
        // Taiwan
        if (getFieldValue('hasPassportIssuedByMFA') === '0' && getFieldValue('passportNotedNationality') === '177')
          bNext = true
        renderByCountryCode = (
          <>
            <VisaRadio
              label={tr(resources.step_prerequisite.hasPassportIssuedByMFA)}
              initialValue={_.get(data, 'hasPassportIssuedByMFA')}
              field="hasPassportIssuedByMFA"
              getFieldDecorator={hasPassportIssuedByMFA}
            />
            ...
            {getFieldValue('hasPassportIssuedByMFA') === '1' && (
              renderLawful()
            )}
          </>
        )
      ...
      default:
        if (flagCountries.includes(countryOfCitizenship)) {
          if (!passportNotedNationality || flagNations.includes(passportNotedNationality))
            bNext = true
          renderByCountryCode = (
            <>
              <Row>
    ...
    return (
      <Form {...formItemLayout}>
        ...
        <div className="visa-global-form-bottom-btn-group">
          ...
          {bNext && showNext && <Button type="primary" onClick={(e) => this.props.handleNext(e, this.props.form, this.handleDates)}>{tr(resources.next)}</Button>}
