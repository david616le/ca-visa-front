...
class AdminBoard extends Component {
  ...
  logout() {
    this.props.cookies.remove("canada_authRedirectTo");
    // this.props.cookies.remove('canada_token')
    ...
    switch (menu) {
      case "ds160":
        if (
          ...
          user.role == constants.USER_ROLE.AGENCY
        )
          renderPage = (
            <AdminPageDS160
      ...
      <Layout className="visa-admin-layout">
        ...
        <Content style={{ padding: "0 50px" }}>
          <Breadcrumb style={{ margin: "16px 0" }}>
            ...
            {menuIndex >= 0 && (
              <Breadcrumb.Item>{menus[menuIndex].label}</Breadcrumb.Item>
            )}
          </Breadcrumb>
...
export default withCookies(
  withRouter(connect(mapStateToProps, mapDispatchToProps)(AdminBoard))
);
