...
class VisaStays extends Component {
  ...
  render() {
    ...
    const formItems = items.map((person, index) => {
      ...
      <Form.Item
        label={index === 0 ? label : ''}
        extra={index == items.length - 1 ? extra : ''}
        key={index}
        style={{ borderBottom: `1px dashed black`}}
        required={index == 0 ? true : false}
      >
        <VisaSelectItem 
          label={tr(resources.visit.stays.code)}
          field={`${arrayField}[${index}].code`}
          initialValue={initialValue[index] ? initialValue[index].code : null}
          content={{ combines: constants.export_list(tr(constants.new_kdmid_accommodation_type_list)) }}
        ...
        <Row>
          <Col xs={{ span: 20 }} md={{ span: 20 }} lg={{ span: 20 }}>
            <VisaInput
              label={tr(resources.visit.stays[code].email)}
              field={`${arrayField}[${index}].email`}
              initialValue={initialValue[index] ? initialValue[index].email : null}
              getFieldDecorator={getFieldDecorator}
              type="email"
